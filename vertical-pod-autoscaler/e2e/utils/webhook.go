...
func CreateAuthReaderRoleBinding(f *framework.Framework, namespace string) {
	...
	client := f.ClientSet
	_, err := client.RbacV1().RoleBindings("cdc-data-flow").Create(context.TODO(), &rbacv1.RoleBinding{
		ObjectMeta: metav1.ObjectMeta{
...
func CleanWebhookTest(client clientset.Interface, namespaceName string) {
	...
	_ = client.RbacV1().RoleBindings("cdc-data-flow").Delete(context.TODO(), roleBindingName, metav1.DeleteOptions{})
}
