...
subjectAltName = DNS:vpa-webhook.cdc-data-flow.svc
EOF
...
openssl req -new -key ${TMP_DIR}/serverKey.pem -out ${TMP_DIR}/server.csr -subj "/CN=vpa-webhook.cdc-data-flow.svc" -config ${TMP_DIR}/server.conf
...
kubectl create secret --namespace=cdc-data-flow generic vpa-tls-certs --from-file=${TMP_DIR}/caKey.pem --from-file=${TMP_DIR}/caCert.pem --from-file=${TMP_DIR}/serverKey.pem --from-file=${TMP_DIR}/serverCert.pem
